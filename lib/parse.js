...
function resolveRelationshipData(included, links) {
  if (!links.data) {
    ...
    return links.data.map(function(item) {
...
function error(object) {
  ...
  var invalid = Object.keys(object).filter(function(key) {
    return VALID_KEYS[key] ? false : true;
  });
    ...
    return [
      {
        ...
        title: "parse error",
        detail:
          "supplied object contains the following invalid key's " +
          invalid.join(", ")
      }
    ];
  }
...
module.exports = function(object) {
  ...
  return object.data instanceof Array
    ? {
        data: object.data.map(function(item) {
          return parseResource(included, item);
        })
      }
    : {
        data: parseResource(included, object.data)
      };
};
